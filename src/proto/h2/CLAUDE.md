...
HTTP/2 client/server integration through the h2 crate, ping handling, and upgrade
support.
...
- `client.rs` - Rust module implementation for client-side connection code.
- `mod.rs` - Module root that wires sibling files into the Rust module tree.
- `ping.rs` - Rust source file for this module or example.
- `server.rs` - Rust module implementation for server-side connection code.
